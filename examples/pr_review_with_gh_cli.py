...
def extract_repo_and_pr_from_url(pr_url: str) -> tuple[str, str, int]:
    ...
    try:
        ...
        if len(path_parts) < 4 or path_parts[2] != "pull":
            raise ValueError("URL must be a GitHub PR URL: https://github.com/owner/repo/pull/123")
...
def fetch_pr_data_with_gh_cli(owner: str, repo: str, pr_number: int) -> dict:
    ...
    try:
        # Get PR details
        pr_cmd = ["gh", "pr", "view", str(pr_number), "--repo", f"{owner}/{repo}", "--json", 
        ...
        if "not found" in e.stderr.lower():
            raise ValueError(f"PR #{pr_number} not found in {owner}/{repo}. Make sure the repository exists and you have access.")
        elif "authentication" in e.stderr.lower():
            raise ValueError("GitHub CLI authentication required. Run: gh auth login")
        else:
            raise ValueError(f"Failed to fetch PR data: {e.stderr}")
    except json.JSONDecodeError:
        raise ValueError("Failed to parse PR data from GitHub CLI")
...
async def create_specialist_agents() -> tuple[Agent, Agent, Agent]:
    ...
    senior_dev_role = RoleDefinition(
        ...
        prompt=(
            ...
            "error handling, testing needs, performance considerations, and maintainability. "
            "Provide constructive, specific feedback in 3-4 sentences. "
            "Focus on code quality and best practices."
        ),
        context={"specialties": ["code_quality", "best_practices", "architecture", "testing"]}
    )
    ...
    security_expert_role = RoleDefinition(
        ...
        prompt=(
            ...
            "Review code for: security vulnerabilities, authentication flaws, data validation, "
            "secret management, encryption standards, injection attacks, and access controls. "
        ...
        context={"specialties": ["security", "encryption", "authentication", "vulnerabilities"]}
    )
    ...
    ux_reviewer_role = RoleDefinition(
        ...
        prompt=(
            ...
            "Review code changes for: user experience implications, accessibility considerations, "
            "error messaging clarity, usability impact, and product requirements alignment. "
            "Consider how technical changes affect end users. "
            "Provide user-centered feedback in 3-4 sentences."
        ),
        context={"specialties": ["user_experience", "accessibility", "usability", "product"]}
    )
...
async def main():
    ...
    if len(sys.argv) != 2:
        ...
        print("   Example: python pr_review_with_gh_cli.py https://github.com/owner/repo/pull/123")
        return 1
    ...
    try:
        ...
        print(f"   ✓ Senior Developer (Code Quality)")
        print(f"   ✓ Security Expert (Vulnerability Analysis)")  
        print(f"   ✓ UX Reviewer (User Experience)")
