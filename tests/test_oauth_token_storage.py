...
class TestOAuthTokenStorage:
    ...
    @pytest.fixture
    def credential_storage(self, tmp_path) -> CredentialStorage:
    ...
    async def test_ensemble_execution_client_id_fallback(self) -> None:
        """Test that ensemble execution uses client_id fallback for anthropic-claude-pro-max."""
        ...
        with pytest.MonkeyPatch.context() as m:
            ...
            m.setattr("llm_orc.ensemble_execution.CredentialStorage", lambda x: mock_storage)
            
            # Mock the OAuthClaudeModel constructor to verify client_id is passed
            oauth_model_calls = []
            
            def mock_oauth_claude_model(*args, **kwargs):
                oauth_model_calls.append(kwargs)
                mock_model = Mock(spec=OAuthClaudeModel)
                return mock_model
            
            m.setattr("llm_orc.ensemble_execution.OAuthClaudeModel", mock_oauth_claude_model)
            ...
            assert oauth_model_calls[0]["client_id"] == "9d1c250a-e61b-44d9-88ed-5944d1962f5e"
            assert oauth_model_calls[0]["access_token"] == "test_access_token"
            assert oauth_model_calls[0]["refresh_token"] == "test_refresh_token"

    @pytest.mark.asyncio
    async def test_ensemble_execution_uses_stored_client_id_when_available(self) -> None:
