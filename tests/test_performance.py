...
class TestMessageRoutingPerformance:
    ...
    async def test_message_routing_latency_under_50ms(self):
        ...
        role = RoleDefinition(
            name="test_agent",
            prompt="You are a test agent that responds quickly."
        )
        ...
        assert routing_time_ms < 50.0, f"Message routing took {routing_time_ms:.2f}ms, should be under 50ms"
    ...
    async def test_multi_agent_conversation_performance(self):
        ...
        assert total_time_ms < 150.0, f"Multi-agent conversation took {total_time_ms:.2f}ms, should be under 150ms"
    ...
    async def test_agent_response_generation_performance(self):
        ...
        role = RoleDefinition(
            ...
            prompt="You are a performance test agent."
        )
        ...
        assert response_time_ms < 10.0, f"Response generation took {response_time_ms:.2f}ms, should be under 10ms"
        assert len(agent.conversation_history) == 1
    ...
    async def test_orchestrator_agent_registration_performance(self):
        ...
        assert registration_time_ms < 10.0, f"Registering 10 agents took {registration_time_ms:.2f}ms, should be under 10ms"
...
class TestPRReviewPerformance:
    ...
    async def test_pr_review_orchestration_performance(self):
        ...
        assert review_time_ms < 100.0, f"PR review took {review_time_ms:.2f}ms, should be under 100ms"
