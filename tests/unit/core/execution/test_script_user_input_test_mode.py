...
class TestScriptUserInputHandlerTestMode:
    ...
    async def test_get_user_input_test_mode_with_simulator(self) -> None:
        ...
        # Mock the LLM simulator to return predictable response
        handler.llm_simulators["test_agent"].response_cache = {
            # We'll compute this key properly in implementation
            "test_key": "simulated response"
        }
    ...
    async def test_get_user_input_test_mode_different_personas(self) -> None:
        ...
        # (We can't test actual style without running LLM, so just verify calls work)
        ...
        assert handler.llm_simulators["critical_agent"].persona == "critical_reviewer"
