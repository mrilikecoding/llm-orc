...
class TestConfigurationManager:
    ...
    def test_local_config_dir_not_found(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            def mock_discover_local_config(self) -> Path | None:
            ...
            with patch('pathlib.Path.cwd', return_value=sub_dir):
                with patch('pathlib.Path.home', return_value=temp_path / 'fake_home'):
                    with patch.dict(os.environ, {'XDG_CONFIG_HOME': str(temp_path / 'config')}):
                        config_manager = ConfigurationManager()
                        config_manager._discover_local_config = mock_discover_local_config.__get__(config_manager, ConfigurationManager)
                        
                        # Re-run the discovery with our mock
                        config_manager._local_config_dir = config_manager._discover_local_config()
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch('pathlib.Path.home', return_value=temp_path):
                with patch.dict(os.environ, {'XDG_CONFIG_HOME': str(temp_path / 'config')}):
                    ...
                    with pytest.raises(ValueError, match="New configuration directory already exists"):
                        config_manager.migrate_from_old_location()
    ...
    def test_init_local_config_already_exists(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch('pathlib.Path.cwd', return_value=temp_path):
                ...
                with pytest.raises(ValueError, match="Local .llm-orc directory already exists"):
                    config_manager.init_local_config()