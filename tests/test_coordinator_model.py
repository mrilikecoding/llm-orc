...
class TestConfigurableCoordinator:
    ...
    def oauth_coordinator_config(self) -> EnsembleConfig:
        ...
        return EnsembleConfig(
            ...
            coordinator={
                ...
                "system_prompt": "You are a strategic advisor with expertise in business analysis.",
                "synthesis_prompt": "Synthesize the expert recommendations into actionable strategy.",
                "timeout_seconds": 60,
            },
        )
    ...
    async def test_get_synthesis_model_uses_configured_model(
        self, executor: EnsembleExecutor, oauth_coordinator_config: EnsembleConfig
    ) -> None:
        ...
        executor._load_model = AsyncMock(return_value=mock_oauth_model)
        ...
        executor._load_model.assert_called_once_with("anthropic-claude-pro-max")
    ...
    async def test_synthesize_results_with_coordinator_role_injection(
        ...
        mock_synthesis_model.generate_response.return_value = "Synthesized result with role"
        ...
        # Call synthesis
        result, model = await executor._synthesize_results(oauth_coordinator_config, agent_results)
        
        # Verify the call
        assert result == "Synthesized result with role"
        assert model == mock_synthesis_model
        
        # Check that generate_response was called with correct role and message structure
        ...
        assert call_args.kwargs["role_prompt"] == "You are a strategic advisor with expertise in business analysis."
        
        # Message should contain synthesis instructions and agent results
        message = call_args.kwargs["message"]
        assert "Synthesize the expert recommendations into actionable strategy." in message
    ...
    async def test_synthesize_results_backward_compatibility(
        ...
        """Test that synthesis works with legacy config (no coordinator system_prompt)."""
        # Mock the synthesis model
        mock_synthesis_model = AsyncMock()
    ...
    async def test_synthesize_results_handles_agent_errors(
        ...
        # Mock the synthesis model
        mock_synthesis_model = AsyncMock()
        mock_synthesis_model.generate_response.return_value = "Synthesis with error handling"
        mock_synthesis_model.get_last_usage.return_value = {
    ...
    async def test_coordinator_model_fallback_on_load_failure(
        ...
        executor._load_model = AsyncMock(side_effect=Exception("Model loading failed"))
    ...
    def test_coordinator_config_validation(self) -> None:
        ...
        config = EnsembleConfig(
            ...
            coordinator={
                ...
                "synthesis_prompt": "Synthesize all perspectives into a final decision.",
                "timeout_seconds": 120,
                "output_format": "json",
            },
        )
    ...
    async def test_coordinator_with_oauth_model_integration(
        ...
        config = EnsembleConfig(
            ...
            coordinator={
                "model": "anthropic-claude-pro-max",
                "system_prompt": "You are a venture capital partner evaluating investments.",
                "synthesis_prompt": "Provide an investment recommendation with risk assessment.",
                "timeout_seconds": 90,
            },
        )
        ...
        mock_oauth_model = AsyncMock(spec=OAuthClaudeModel)
        mock_oauth_model.generate_response.return_value = "INVESTMENT RECOMMENDATION: PASS - High risk, insufficient market validation."
        mock_oauth_model.get_last_usage.return_value = {
            "input_tokens": 200, "output_tokens": 100, "cost_usd": 0.0
        }
        ...
        agent_results = {
            "market_analyst": {"status": "success", "response": "Market size: $2B TAM, growing 15% annually"},
            "financial_analyst": {"status": "success", "response": "Unit economics unclear, high burn rate"},
            "tech_analyst": {"status": "success", "response": "Strong technical team, but unproven architecture"},
        }
