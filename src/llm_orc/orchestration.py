...
class ConversationOrchestrator:
    ...
    async def start_conversation(self, participants: list[str], topic: str, initial_message: str = None) -> str:
    ...
    async def send_agent_message(self, sender: str, recipient: str, content: str, conversation_id: str) -> str:
    ...
    async def review_pr(self, pr_data: dict) -> dict:
        ...
        for reviewer_name, reviewer_agent in self.reviewers.items():
            ...
            reviews.append({
                ...
                "feedback": feedback,
                "specialization": reviewer_agent.role.context.get("specialties", [])
            })
...
""".strip()
    ...
    async def _generate_summary(self, reviews: list, pr_data: dict) -> str:
        ...
        for review in reviews:
            ...
            if "security" in feedback or "vulnerability" in feedback or "critical" in feedback:
                security_issues.append(review["reviewer"])
            if "code quality" in feedback or "best practices" in feedback or "testing" in feedback:
        ...
        summary_parts = []
        if security_issues:
            summary_parts.append(f"🔒 Security concerns raised by: {', '.join(security_issues)}")
        if code_quality_issues:
            summary_parts.append(f"📝 Code quality feedback from: {', '.join(code_quality_issues)}")
        if ux_issues:
            summary_parts.append(f"👤 UX considerations noted by: {', '.join(ux_issues)}")
        
        if not summary_parts:
            summary_parts.append("✅ No major issues identified across all review areas")
