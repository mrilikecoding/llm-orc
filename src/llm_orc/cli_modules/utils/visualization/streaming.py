...
from .results_display import display_results, display_plain_text_results
...
async def run_standard_execution(
    ...
    if output_format == "json":
        ...
        agents = ensemble_config.agents
        display_results(result["results"], result["metadata"], agents, detailed=detailed)
...
async def _run_rich_streaming_execution(
    ...
    with Live(
        create_dependency_tree(agents, {name: "pending" for name in agent_progress}),
        console=console,
        ...
        try:
            async for event in execute_stream():
                ...
                statuses = {name: info["status"] for name, info in agent_progress.items()}
...
def _handle_streaming_event(event: dict[str, Any], agent_progress: dict[str, dict[str, Any]]) -> None:
...
def _handle_fallback_started_event(
    ...
    if agent_name:
        _update_agent_progress_status(
            agent_name, f"🔄 Fallback: {original_model} → {fallback_model}", agent_progress
        )
...
def _handle_text_fallback_started(event: dict[str, Any]) -> None:
    ...
    click.echo(f"🔄 {agent_name}: Falling back from {original_model} to {fallback_model}")
