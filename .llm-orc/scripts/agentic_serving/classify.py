...
is non-build prose; everything else is a build routed to the default
code-generation seat. The build-vs-non-build (executable-deliverable)
determination is classify's own responsibility (ADR-046 §1 responsibility
matrix) — ``build`` gates marshal's file-vs-prose shaping and, at WP-D8, the
gated accept shape.

The seat is filled by dynamic dispatch on ``${classify.target}``, so swapping a
seat strategy is a change to this decision or the operator default, never to the
skeleton (AS-11). A model-backed decider for turns whose routing is not
structural (scenarios.md "classify reads intent with a model-backed decider")
is a later WP-A8 node; this node covers the deterministic path.
"""
...
def main() -> None:
    ...
    if is_explain:
        target, kind, build = _EXPLAIN_SEAT, "explanation", False
    else:
        target = _DEFAULT_CODE_SEAT
        kind = turn.get("kind", "python_module")
        build = True

    file = turn.get("file") or _extract_file(task) or "solution.py"
