...
Primitive scripts are resolvable at run time from the installed `llm_orc.primitives` package — a script agent referencing `primitives/...` resolves against the package regardless of `llm-orchestra-library/` or any environment variable (`ScriptResolver`'s "Priority 1.5: Installed package primitives"). But `llm-orc scripts list` does NOT enumerate them: it only scans `.llm-orc/scripts/primitives/` and `llm-orchestra-library/scripts/primitives/` (`PrimitiveRegistry.discover_primitives`), so a fresh `llm-orc init` with no environment variables set and neither directory populated reports "No scripts found." even though the built-in primitives are usable by script agents. `llm-orc init` does not copy anything from `llm-orchestra-library/`, and `LLM_ORC_LIBRARY_SOURCE` has no effect on any of this. `LLM_ORC_LIBRARY_SOURCE=local` in particular is a no-op everywhere in the current CLI — see [Library Source Configuration](#library-source-configuration) above, where the same variable's only remaining meaning (`=remote`) is documented for the `llm-orc library` command group.
