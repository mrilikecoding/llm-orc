...
## Work Packages — Cycle 8 (BUILD complete; the declarative-ensemble collapse)

> **Cycle-8 supersession (ARCHITECT 2026-07-02; BUILD complete 2026-07-08).** The clean-slate collapse (ADR-045/046) dissolved the imperative orchestrator-actor serving architecture. The Cycle-4/6/7 work packages that described that dissolved form are retained as historical record and now live under the Completed Work Log (see §Historical Record), migrated at Cycle-8 BUILD alongside the `agentic/` code deletion (WP-F8). Derived from `system-design.md` §Cycle 8 and ADR-044/045/046/047/048.

**All six Cycle-8 work packages (WP-A8, WP-B8, WP-C8, WP-D8, WP-E8, WP-F8) are complete.** Their objectives, commit references, and closing summaries live in the Completed Work Log (§Cycle 8). The per-turn handler is now ONE declarative ensemble (classify → seat → shape / form-gate / emit) on the shipped primitives; the Topaz-keyed capability registry, operator-curated Shape Catalog, composed Accept Gate, and seat-contract wiring are all in place; and `src/llm_orc/agentic/` (~12.7K LOC, 32 files) has been deleted with parity grounded through the real L3 endpoint.

**Next step (graduate-time, terminal):** merge a clean `main` (native docs only) and archive the full RDD Cycle-8 corpus on a research branch. No further BUILD work remains in Cycle 8.
...
**Reached (WP-A8, 2026-07-02).** The serving ensemble serves a build turn end-to-end (classify → seat → marshal) on the new declarative form with a stubbed registry and no accept gate. Coherent and demonstrable, narrower than target, and it validates the collapse's central claim (one ensemble on the shipped engine) before the registry, the gate, or the deletion.

### TS-2 (Cycle 8): parity reached (after WP-A8/C8/D8/E8)

**Reached (WP-A8/C8/D8/E8, 2026-07-08); the deletion (WP-F8) is done.** Registry + accept gate + seat-contract wiring in place; the new form matches the narrow Cycle-7 serve (build-new-files) and extends toward general-turn parity. The old `agentic/` layer has now been deleted (WP-F8, 2026-07-08) with no capability loss. Parity-before-delete met.
...
### Cycle 8: The declarative-ensemble collapse (closed 2026-07-08)

**Derived from:** ADRs 044-048, `system-design.md` §Cycle 8

**Summary:** All six work packages (WP-A8 through WP-F8) closed 2026-07-02 to 2026-07-08. The per-turn handler is one declarative ensemble on the shipped primitives; TS-2 parity was reached (WP-A8/C8/D8/E8) and the imperative `src/llm_orc/agentic/` layer (~12.7K LOC, 32 files) was deleted at WP-F8, grounded through the real L3 endpoint. Per-WP detail follows the table.
...
| WP-C8 | Shape Catalog + registry reuse (Topaz-keyed parts + AS-2 admission) | 2026-07-08 | `20b06c1`, `eb0a10f`, `003dfd2` | Complete |
| WP-E8 | Seat-contract wiring (`ValidationEvaluator` as the seat pass/fail gate) | 2026-07-08 | `5dbea09`, `dc4f2b5`, `6d69c79` | Complete |
| WP-F8 | Clean-slate removal — `agentic/` deletion + Cycle-7 serving-ADR supersession + current-state sweep | 2026-07-08 | `53c86cb`, `3fd9272`, `570f1ca`, `6974176` | Complete |

*Per-WP detail (objectives and closing summaries, migrated from the active Work Packages section at Cycle-8 BUILD):*
...
**Status: COMPLETE 2026-07-08** (three commits: Topaz-keyed registry + AS-2 admission; Shape Catalog + retire hardcoded `_SHAPE`; gen→review exemplar). New L1 `src/llm_orc/core/serving/` package derived from the ensemble library (AS-11 — no parallel structure): `admission.py` (the single AS-2 registration gate reusing `validate_ensemble_reference_graph` + depth), `capability_registry.py` (`capability_parts()` — parts Topaz-keyed, shapes excluded), `shape_catalog.py` (`shape_catalog()` — intent→shape from a new `serves` EnsembleConfig field). `resolve.py` reads the catalog (routing unchanged — parity — but operator-curated). Scenario-4 slot-filling: the `gen→review` exemplar shape fills both slots by dynamic dispatch resolving REGISTERED parts from the registry via a deterministic `select_parts` selector (shipped primitives; registry now live). **Good defaults + extensibility** settled at the WP gate: framework ships good defaults, operators extend seats (registry) + flows (catalog); the verification-shape map is realized by good defaults, not a runtime rung-check. Grounded both ways (gen-review invoke; real serve build turn). 2941 unit green, loop-driver byte-untouched, mypy/ruff clean. **Parity-before-delete now MET** (A8/C8/D8/E8). See cycle-status §WP-C8; gate note `.rdd/gates/cycle-8-c8-gate.md`.
...
**Status: COMPLETE 2026-07-08** (commits `53c86cb` step 0, relocate the `/v1/models` allow-list to `core/config/`; `3fd9272` step 1, sever the calibration channel from the L0 engine; `570f1ca` step 2, remove the loop-driver serving path so declarative is the only serving path; `6974176` step 3+4, delete `src/llm_orc/agentic/` [32 files, ~12.7K LOC] plus 10 re-export shims, ~43 dissolving tests removed and 10 survivors migrated). The ADR supersession headers (033/036/037/039/041/043) were already applied in the DECIDE sweep. Verified: 2307 unit + 31 integration green (1 known pre-existing env failure); grounded through the real L3 endpoint both ways: a build turn (a `write` tool_call carrying correct `fib(n)` through the 8-node declarative pipeline) and an explain turn (prose). This is the last Cycle-8 WP; with it Cycle-8 BUILD is complete and the imperative `agentic/` layer is gone.
...
## Historical Record: Dissolved Imperative Serving Architecture (Cycles 4, 6, 7)

> **Migrated to the Completed Work Log at Cycle-8 BUILD (2026-07-08).** The sections below are the forward-looking sequencing surfaces for Cycles 4, 6, and 7. They describe the imperative orchestrator-actor serving architecture that the Cycle-8 declarative-ensemble collapse (ADR-045/046) dissolved, and are retained verbatim as historical record. The code they describe (`src/llm_orc/agentic/`) was deleted at WP-F8 (2026-07-08). Nothing here is active work.
...
## Work Packages — Cycle 7 loop-back (historical; tool-driven multi-turn surface)
...
## Work Packages — Cycle 7 single-turn surface (historical; carried across the loop-back)
...
## Work Packages — Cycle 7 (historical)
