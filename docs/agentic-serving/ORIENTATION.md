...
**RESEARCH, DISCOVER, MODEL, DECIDE, and ARCHITECT phases complete. BUILD in progress — WP-A through WP-G done. TS-1 (stateless orchestrator serving OpenCode) reached at WP-F close; TS-2 (stateless baseline complete) now one WP away. Seven of ten work packages complete.**
...
**TS-1 reached.** An operator can deploy llm-orc with the Serving Layer, point OpenCode (or any OpenAI-compat agentic coding tool) at `/v1/chat/completions`, and run an RDD phase through it — the orchestrator routes tasks to library ensembles, summarizes results, enforces Budget, delegates client-side actions at turn boundaries, and retries composed ensembles whose mid-execution needs were un-predicted.

**Next WPs (toward TS-2, TS-3):**
- **WP-H: Calibration Gate** — the last TS-2 item. Interposes Calibration on `invoke_ensemble` for composed ensembles in their first N invocations; closes FC-12. Open decisions: default N, check mechanism (may itself be an ensemble).
- **WP-I: Plexus Adapter (tool-first)** — begins TS-3. Wires `query_knowledge` and `record_outcome` with graceful no-op fallbacks when Plexus is absent. Open decision: `record_outcome` payload schema.

WP-H depends on WP-G (complete — composed ensembles now exist to calibrate). WP-I depends only on WP-C (already complete). Both advance toward TS-2 and TS-3; WP-H lands closer to TS-2 completion.
...
- **Silent quality failures when retry convention not honored** → WP-H Calibration Gate scope (see cycle-status FF #107).
- **Summarizer quality / echo-back risk** → WP-H calibration scope (see cycle-status FF #81).
- **Calibration N** (WP-H); **`record_outcome` payload schema** (WP-I).
