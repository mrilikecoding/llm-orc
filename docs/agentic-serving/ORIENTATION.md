...
**RESEARCH, DISCOVER, MODEL, DECIDE, and ARCHITECT phases complete.** The next phase is **BUILD**.
...
**Open going into BUILD:** client-tool BDD scenarios (blocks WP-F); visibility form (OQ #2); Budget specific numbers; Calibration N; Session identity mechanism; `record_outcome` payload schema. See `roadmap.md` §Open Decision Points for full list and which WPs they gate.
