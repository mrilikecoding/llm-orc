...
| BUILD | ▶ In Progress (entered 2026-05-11; **WP-A4 closed 2026-05-11**, commits `cc0d94f` + `7c2f64e`) | `src/llm_orc/models/structural_errors.py` (new); `src/llm_orc/models/base.py` (ToolCallingNotSupportedError re-parented); `tests/unit/models/test_structural_errors.py` (new — 12 tests; full suite 2363 passing) | WP-A4 (shared `LlmOrcStructuralError` base class) — first concrete subclass (`ToolCallingNotSupportedError`) migrated with no-regression behavior; FC-17 coverage 1 of 8 typed-error surfaces (remaining 7 land in WP-C4 through WP-H4); Tier 1 stewardship check clean; `NotImplementedError` lineage dropped (verified no caller depends on it — see commit message). Next WP: practitioner-default to **WP-B4** (FC-2 + FC-3 automated tests; mutually independent of WP-A4 per roadmap) before WP-C4 hard-dependent producers begin landing. |
