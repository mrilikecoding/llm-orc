...
| L1 — Domain Policy | Composition Validator, Budget Controller, Autonomy Policy, Calibration Gate, Plexus Adapter, Calibration Signal Channel *(Cycle 4; conditional acceptance)*, **Dispatch Event Substrate** *(new in Cycle 6)*, **Session Artifact Store** *(new in Cycle 6)* | May depend on L0 only — **plus an intra-L1 producer-substrate edge:** the four event-producing L1 modules (Calibration Gate, Tier-Escalation Router (resides at L2 — see note below), Tier-Router-Audit, Calibration Signal Channel) emit events through the Dispatch Event Substrate's `emit()`; Session Artifact Store reads from Dispatch Event Substrate for `dispatch_id` lifecycle. Intra-L1 edges do not form cycles. |
| L2 — Runtime | Result Summarizer Harness, Orchestrator Tool Dispatch, Orchestrator Runtime, Conversation Compaction *(Cycle 4)*, Tier-Escalation Router *(Cycle 4)*, **Orchestrator-Context Event Sink** *(new in Cycle 6)* | May depend on L0 and L1 |
| L3 — Entry | Serving Layer, Session Registry, Bootstrapping Pipeline, Orchestrator Configuration, **Operator-Terminal Event Sink** *(new in Cycle 6)* | May depend on L0, L1, and L2 |
